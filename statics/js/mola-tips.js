...
re.toString = function() {
    showMessage('喵呜，控制台被打开啦，我要被抓走啦', 5000, true);
    return '';
};

$(document).on('copy', function (){
    showMessage('你都复制了些什么呀，复制粘贴也要注意格式呀', 5000, true);
});
...
(function (){
    ...
    if (window.location.href == SiteIndexUrl) {      // 如果是主页
        ...
        if (now > 23 || now <= 5) {
            ...
            text = '早上好，今天好像依旧还是很困呢';
        } else if (now > 9 && now <= 11) {
            text = '太阳高照！工作顺利嘛，不要久坐，多起来走动走动哦！';
        } else if (now > 11 && now <= 14) {
            text = '吃饭啦！今天是点外卖还是去食堂呢！';
...
function showHitokoto(){
    ...
    $.getJSON('https://v1.hitokoto.cn',function(result){
        var text = '这句一言来自 <span style="color:#6363D0;">『{source}』</span>，是 <span style="color:#6363D0;">{creator}</span> 在 hitokoto.cn 投稿的。';
        text = text.render({source: result.from, creator: result.creator});
        showMessage(result.hitokoto, 5000);
        window.setTimeout(function() {showMessage(text, 3000);}, 5000);
    });
	/*
	$.getJSON('https://api3.fghrsh.net/hitokoto/rand/?encode=jsc&uid=3335',function(result){
...
function loadModel(modelId, modelTexturesId){
    ...
    loadlive2d('live2d', 'http://www.molapages.xyz/molablog/client/live2d/get/?id='+modelId+'-'+modelTexturesId, console.log('live2d','模型 '+modelId+'-'+modelTexturesId+' 加载完成'));
}
...
function loadRandModel(){
    ...
    $.ajax({
        cache: false,
        url: 'http://www.molapages.xyz/molablog/client/live2d/'+modelTexturesRandMode+'_textures/?id='+modelId+'-'+modelTexturesId,
        dataType: "json",
        success: function (result){
...
function loadOtherModel(){
    ...
    $.ajax({
        cache: false,
        url: 'http://www.molapages.xyz/molablog/client/live2d/'+modelTexturesRandMode+'/?id='+modelId,
        dataType: "json",
        success: function (result){
